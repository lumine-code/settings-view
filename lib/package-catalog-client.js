...
function defaultCachePath() {
  try {
    return path.join(lumine.app.getPath("userData"), "Cache", "settings-view");
  } catch {
...
module.exports = class PackageCatalogClient {
  constructor({
    ...
    lumineVersion = () => (global.lumine && lumine.app.getVersion ? lumine.app.getVersion() : null),
  } = {}) {
  ...
  validate(value, source = null) {
    if (!Array.isArray(value)) {
      ...
      throw new Error("A package catalog must be a JSON array of Git sources.");
    }
    return value.map((entry, index) => {
      if (typeof entry !== "string") {
        throw new Error(`Catalog entry ${index + 1} must be a Git source string.`);
      }
