...
module.exports = class PackageManager {
  constructor() {
    ...
    this.catalogUpdateOrigins = new Set();
  ...
  packageHasSettings(packageName) {
    ...
    for (let grammar of grammars) {
      if (grammar.packageName === packageName && grammar.name && grammar.scopeName) return true;
    }
  ...
  loadOutdated(clearCache, callback) {
    if (clearCache) {
      ...
      return callback(null, this.getAvailableUpdates());
    }

    this.getGitPackageUpdates().then((updatablePackages) => {
      // getGitPackageUpdates() owns the live implementation and updates this
      // state itself. Keep this assignment here as well so tests and embedders
      // that replace it with their own checker still get the same contract.
      this.replaceAvailableUpdates(updatablePackages);
  ...
  onDidChangeAvailableUpdates(callback) {
    return this.emitter.on("did-change-available-updates", callback);
  }

  updateStateFingerprint(pack) {
  ...
  availableUpdateMapsEqual(left, right) {
    if (left.size !== right.size) return false;
    for (const [originKey, update] of left) {
      const other = right.get(originKey);
      if (!other || this.updateStateFingerprint(update) !== this.updateStateFingerprint(other)) {
        return false;
      }
    }
    return true;
  ...
  setAvailableUpdateMap(updates) {
    if (this.availableUpdateMapsEqual(this.availableUpdates, updates)) {
      this.availableUpdates = updates;
      return false;
    }
    this.availableUpdates = updates;
    this.emitter.emit("did-change-available-updates", this.getAvailableUpdates());
    return true;
  }

  replaceAvailableUpdates(packs) {
    const updates = new Map();
    for (const pack of packs || []) {
      const originKey = packageOrigin(pack);
      if (originKey && !updates.has(originKey)) updates.set(originKey, pack);
    }
    this.catalogUpdateOrigins.clear();
    this.setAvailableUpdateMap(updates);
    return this.getAvailableUpdates();
  }

  removeAvailableUpdate(pack) {
    const originKey = packageOrigin(pack);
    if (!originKey || !this.availableUpdates.has(originKey)) return false;
    const updates = new Map(this.availableUpdates);
    updates.delete(originKey);
    this.catalogUpdateOrigins.delete(originKey);
    return this.setAvailableUpdateMap(updates);
  }

  // Accept the already-validated, flattened records returned by the catalog
  // client. A catalog snapshot is authoritative for the exact release commit
  // it advertises: do not re-resolve its tag or fetch its manifest here. Other
  // update policies remain the responsibility of the live checker below.
  mergeCatalogUpdates(catalogPackages) {
    const catalogByOrigin = new Map();
    for (const pack of catalogPackages || []) {
      const originKey = packageOrigin(pack);
      if (originKey && !catalogByOrigin.has(originKey)) catalogByOrigin.set(originKey, pack);
    }

    const catalogUpdates = new Map();
    const gitPackages = this.getLocalPackages().git.filter((pack) => !pack.isShadowed);
    for (const installed of gitPackages) {
      const install = installed.apmInstallSource;
      if (!install || install.updatePolicy !== "latest-tag") continue;

      const originKey = packageOrigin(installed);
      const available = originKey && catalogByOrigin.get(originKey);
      const latest = available && available.refs && available.refs.latestStable;
      if (!this.isUsableCatalogUpdate(installed, available, latest)) continue;
      ...
      catalogUpdates.set(
        ...
        _.extend({}, installed, {
          ...
          resolvedRef: { type: "latest", value: latest.name },
        }),
      );
    }

    const updates = new Map(this.availableUpdates);
    for (const originKey of this.catalogUpdateOrigins) updates.delete(originKey);
    this.catalogUpdateOrigins = new Set(catalogUpdates.keys());
    for (const [originKey, pack] of catalogUpdates) updates.set(originKey, pack);
    this.setAvailableUpdateMap(updates);
    return this.getAvailableUpdates();
  }

  isUsableCatalogUpdate(installed, available, latest) {
    if (!available || available.status !== "ready" || !latest) return false;
    if (available.name !== installed.name) return false;
    if (typeof latest.name !== "string" || !latest.name) return false;
    if (!/^[0-9a-f]{40}$/i.test(latest.sha || "")) return false;
    if ((available.resolvedSha || "").toLowerCase() !== latest.sha.toLowerCase()) return false;
    const installedSha = installed.apmInstallSource && installed.apmInstallSource.sha;
    if ((installedSha || "").toLowerCase() === latest.sha.toLowerCase()) return false;

    const installedVersion = semver.valid(installed.version);
    const latestVersion = semver.valid(latest.version);
    const manifestVersion = semver.valid(available.version);
    if (!installedVersion || !latestVersion || !manifestVersion) return false;
    if (semver.prerelease(latestVersion) || !semver.eq(manifestVersion, latestVersion)) {
      return false;
    }
    if (!semver.gt(latestVersion, installedVersion)) return false;

    const lumineVersion = this.normalizeVersion(lumine.application.getVersion());
    return this.satisfiesVersion(lumineVersion, available);
  }
  ...
  update(pack, newVersion, callback) {
    ...
    this.installGitHubPackage(exactUpdate).then(
      (updatedPack) => {
        this.clearOutdatedCache();
        this.removeAvailableUpdate(updatedPack);
  ...
  install(pack, callback, options = {}) {
    ...
    this.installGitHubPackage(pack, options).then(
      (installedPack) => {
        ...
        this.clearOutdatedCache();
        this.removeAvailableUpdate(pack);
  ...
  getPackageTitle({ name }) {
    return _.titleize(_.uncamelcase(name));
  }
  ...
  async getGitPackageUpdates() {
    ...
    this.replaceAvailableUpdates(updates);
    return updates;
  }
