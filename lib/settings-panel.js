...
module.exports = class SettingsPanel extends CollapsibleSectionPanel {
  constructor(options = {}) {
    ...
    this.disposables.add(scopeContext.onDidChange(() => this.reload()));
    this.reload();
  }
  ...
  elementForSettings(namespace, settings) {
    ...
    container.appendChild(this.scopeSelectorElement());
  ...
  getKnownScopeSelectors() {
    ...
    for (const grammar of lumine.grammars.getGrammars({ includeTreeSitter: true })) {
  ...
  getScopePredefinedValues({ loading = false } = {}) {
    return [
      { value: "", label: "Default" },
      ...(loading
        ? []
        : this.getKnownScopeSelectors().map((selector) => ({
            value: selector,
            label: selector,
          }))),
    ];
  ...
  scopeSelectorElement() {
    ...
    const selectorEditor = new PredefinedValuesEditor({
      ...
      values: this.getScopePredefinedValues({ loading }),
      loading,
    ...
    explanation.classList.add("text-subtle", "settings-scope-explanation");
    explanation.textContent = current
      ? `Changes create user overrides stored under ${current}.`
      : 'Changes edit the base values stored under "*".';
  ...
  bindScopeSelector() {
    ...
    const refreshScopeOptions = () => {
      ...
      selectorEditor.setValues(this.getScopePredefinedValues({ loading }), { loading });
    };
  ...
  observe(name, callback) {
    const read = () => {
      const selector = scopeContext.get();
      if (!selector) return getWithoutProjectOverride(name);
      const inspection = lumine.config.inspect(name, { scopeSelector: selector });
      return inspection.hasOverride ? inspection.overrideValue : valueForSelector(name, selector);
    };
    callback(read());
    this.renderDisposables.add(
  ...
  isDefault(name) {
    const selector = scopeContext.get();
    if (selector) return !lumine.config.inspect(name, { scopeSelector: selector }).hasOverride;
    let params = { sources: [lumine.config.getUserConfigPath()] };
    let defaultValue = this.getDefault(name);
    let value = lumine.config.get(name, params);
  ...
  getDefault(name) {
    ...
    if (selector) {
      const inspection = lumine.config.inspect(name, { scopeSelector: selector });
      return inspection.variableByMatch ? inspection.baseValue : inspection.inheritedValue;
    }
    let params = { excludeSources: [lumine.config.getUserConfigPath()] };

    let defaultValue = lumine.config.get(name, params);
    return defaultValue;
  }
  ...
  bindTooltips() {
    const disposables = Array.from(
      this.element.querySelectorAll("input[id], select[id], lumine-text-editor[id]"),
    ).map((element) => {
...
function elementForSetting(namespace, name, value, context = {}) {
  ...
  const scopedLeaf = context.selector && schema?.type !== "object";
  if (scopedLeaf) {
    ...
    toggle.checked = context.inspection.hasOverride;
    toggle.setAttribute("aria-label", `Override ${keyPath} in ${context.selector}`);
    toggle.title = `Override ${keyPath} in ${context.selector}`;
    controlGroup.insertBefore(toggle, controls);
    if (!context.inspection.hasOverride) {
  ...
  if (scopedLeaf && !context.inspection.hasOverride) {
    for (const input of controls.querySelectorAll("input, select, button")) input.disabled = true;
    for (const editorElement of controls.querySelectorAll("lumine-text-editor")) {
