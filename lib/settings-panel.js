...
module.exports = class SettingsPanel extends CollapsibleSectionPanel {
  constructor(options = {}) {
    ...
    if (this.options.scopeName) {
      namespace = "language";
      settings = {};
      const schema = lumine.config.getSchema("language");
  ...
  observe(name, callback) {
    ...
    if (lumine.config.projectFile) {
      params.excludeSources = [lumine.config.projectFile];
    }
    ...
    this.disposables.add(lumine.config.observe(name, params, wrappedCallback));
  }
  ...
  getDefault(name) {
    ...
    if (this.options.scopeName != null) {
      ...
      if (
        lumine.config.get(name, { excludeSources: [lumine.config.getUserConfigPath()] }) ===
        defaultValue
      ) {
        defaultValue = lumine.config.get(name);
      }
    }
    return defaultValue;
  }
  ...
  set(name, value) {
    if (this.options.scopeName) {
      if (value === undefined) {
        lumine.config.unset(name, { scopeSelector: this.options.scopeName });
        return true;
      } else {
        return lumine.config.set(name, value, { scopeSelector: this.options.scopeName });
      }
    } else {
  ...
  bindEditors() {
    const disposables = Array.from(this.element.querySelectorAll("lumine-text-editor")).map(
      (editorElement) => {
...
function elementForSetting(namespace, name, value) {
  ...
  let schema = lumine.config.getSchema(`${namespace}.${name}`);
