...
const getInstalledPackageMetadata = (name) => {
  ...
  const availablePackage = lumine.packages.getAvailablePackage(name);
...
const packageComparatorAscending = (left, right) => {
  const leftStatus = lumine.packages.isPackageDisabled(left.name);
  const rightStatus = lumine.packages.isPackageDisabled(right.name);
